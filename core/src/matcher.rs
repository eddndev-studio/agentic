...
pub fn find_match(content: &str, triggers: &[Trigger]) -> Option<MatchResult> {
    ...
    for t in triggers {
        if t.match_type == "EXACT" && t.keyword.to_lowercase() == normalized {
    ...
    // 2. CONTAINS matches
    for t in triggers {
        if t.match_type == "CONTAINS" && normalized.contains(&t.keyword.to_lowercase()) {
            return Some(MatchResult {
                trigger: t.clone(),
            });
        }
...
mod tests {
    ...
    fn exact_match_case_insensitive() {
        let triggers = vec![make_trigger("hello", "EXACT")];
    ...
    fn exact_has_priority_over_contains() {
        let triggers = vec![
            make_trigger("hello", "CONTAINS"),
            make_trigger("hello", "EXACT"),
        ];
        let result = find_match("hello", &triggers);
        assert!(result.is_some());
